...
function printContent() {
	$noDispArr=array("olks.cfg","framework.cfg");
	$rlink=_link("modules")."&mod={$_ENV['CURRENT_MODULE']}&popup=true&cfg=";
	if(file_exists(APPROOT."config/cfgGroups.php")) include APPROOT."config/cfgGroups.php";
	else {
		$cfgGroups=array(); $cfgGroupsFor="";
		$cfgGroupsDefaulters=array();
	}
?>
<style>
.tabPage {
		...
		<select id=reportselector size=2 onchange='loadConfig();' class='ui-widget-content ui-corner-all'>
			<?php
				...
				if(is_dir($f)) {
					...
					if(count($fs)>0) echo "<optgroup label='Logiks Core Settings' class='clr_blue'>";
					foreach($fs as $lf) {
						if(isset($cfgGroupsFor) && $cfgGroupsFor==$lf) continue;
						if(is_file($f.$lf) && strtolower(strstr($lf,"."))==".cfg" && !in_array($lf,$noDispArr) && !in_array($lf,$cfgGroupsDefaulters)) {
				...
				$f=ROOT.CFG_FOLDER."/features/";
				if(is_dir($f)) {
					$fs=scandir($f);
					unset($fs[0]);unset($fs[1]);
					if(count($fs)>0) echo "<optgroup label='Feature Settings' class='clr_blue'>";
					foreach($fs as $lf) {
				...
				echo "<optgroup label='Default Settings' class='clr_blue'>";
				foreach($cfgGroupsDefaulters as $x) {
					$x=str_replace('.cfg','',$x);
					$t=_ling($x."_Settings");
					$t=toTitle($t);
					echo "<option value='$x'>".$t."</option>";
				...
				echo "</optgroup>";
				echo "<optgroup label='System Settings' class='clr_pink'>";
				$cfgGroups=array_keys($cfgGroups);
