...
$a=file_put_contents(ROOT.BACKUP_FOLDER."/.htaccess","deny from all\n");
if($a>0)
...
cmdLnk=getServiceCMD("sitebackup");
$(function() {
	$("select#siteselector").load(getServiceCMD("qtools")+"&action=sitelist&format=select");
	reloadList();
});
function reloadList() {
	$("#loadingmsg").show();
	...
	$("#datatable tbody").load(lnk,function() {
			...
			$("#datatable .backup .buttondiv").click(function() {
					...
					if($(this).hasClass("downloadicon")) {
						q=getServiceCMD("sitebackup")+"&action=download&ref="+ref;
						window.open(q);
					} else if($(this).hasClass("restoreicon")) {
						$("#loadingmsg").show();
						q=getServiceCMD("sitebackup")+"&action=restore&ref="+ref+"&forsite="+site;
						processAJAXQuery(q,function(txt) {
								if(txt.length>0) lgksAlert(txt);
								$("#loadingmsg").hide();
							});
					} else  if($(this).hasClass("deleteicon")) {
						q=getServiceCMD("sitebackup")+"&action=delete&ref="+ref;
						$("#loadingmsg").show();
						processAJAXQuery(q,function(txt) {
								if(txt.length>0) lgksAlert(txt);
								reloadList();
