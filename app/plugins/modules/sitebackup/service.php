...
function getDBControls($site) {
	...
	if(file_exists($dbFile)) {
		...
		if(strlen($GLOBALS['DBCONFIG']["DB_USER"])>0 && strlen($GLOBALS['DBCONFIG']["DB_HOST"])>0 &&
			strlen($GLOBALS['DBCONFIG']["DB_DATABASE"])>0 && strlen($GLOBALS['DBCONFIG']["DB_DRIVER"])>0) {
...
function backup_tables($dir,$tables = '*'){
	$link =getDBControls($_REQUEST["forsite"]);
	if($link==null || $link===false) return false;
